...
from musicApp.models import Song , Watchlater, History , Channel
...
def search(request):
    ...
    return render(request,'musicApp/search.html',{"songs":qs})
...
def history(request):
    if request.method =="POST":
        ...
        return redirect(f"/musicApp/songs/{music_id}")
    ...
    preserved = Case(*[When(pk=pk, then=pos) for 
                       pos, pk in enumerate(ids)])
    song = Song.objects.filter(song_id__in=ids).order_by(preserved)
    return render(request,'musicApp/history.html',{"history":song})
...
def watchlater(request):
     if request.method == "POST":
        ...
        return render(request, f"musicApp/songpost.html", {'song': song, "message": message})
     ...
     return render(request, 'musicApp/watchlater.html')
...
def songs(request):
    song = Song.objects.all()
    return render(request,'musicApp/songs.html' , {'song': song}) 
def songpost(request,id):
    song = Song.objects.filter(song_id =id).first()
    return render(request,'musicApp/songpost.html',{'song': song})
...
def channel(request, channel):
    ...
    return render(request, "musicApp/channel.html", {"channel": chan, "song": song})
